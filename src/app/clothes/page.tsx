...
export default async function ClothesHubPage({ searchParams }: SearchParamsProps) {
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 space-y-10">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-6">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="text-slate-400 uppercase tracking-wider">Size:</span>
          <div className="flex gap-1">
            <Link
              href={`/clothes${categoryFilter ? `?category=${categoryFilter}` : ''}`}
            ...
            {clothingSizes.map((s) => (
              <Link
                key={s.id}
                href={`/clothes?size=${s.code}${categoryFilter ? `&category=${categoryFilter}` : ''}`}
      ...
      {products.length === 0 ? (
        <div className="py-20 text-center text-slate-400 space-y-3 border border-white/5 bg-[#0A192F]">
          <p className="font-serif text-lg text-white">No garments match your selected filters</p>
          <Link href="/clothes" className="inline-block mt-4 px-6 py-2.5 bg-white text-[#0A192F] font-bold text-xs uppercase">
            Reset Filters
          </Link>
        </div>
      ) : (
              ...
              >
                <div>
                  ...
                  <div className="p-2.5 sm:p-4 space-y-1.5 sm:space-y-2">
                    ...
                    <p className="text-[11px] sm:text-xs text-slate-400 line-clamp-2 leading-relaxed hidden sm:block">{prod.description}</p>
